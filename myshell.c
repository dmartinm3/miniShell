...
typedef enum { trabajoEnEjec = 0, trabajoTerminado = 1 } estadoTrabajo_t;
...
/* La shell ignora Ctrl+C/Ctrl+\ para no morir con el teclado. */
...
static void actualizarTrabajosNoBloqueante(void) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        for (int j = 0; j < MAX_TRABAJOS; ++j) {
            if (!trabajos[j].usado) continue;
            for (int k = 0; k < trabajos[j].numPids; ++k) {
                if (trabajos[j].pids[k] == pid) { trabajos[j].pids[k] = 0; break; }
            }
        }
    }
    for (int j = 0; j < MAX_TRABAJOS; ++j) {
        if (!trabajos[j].usado || trabajos[j].estado == trabajoTerminado) continue;
        bool algunoVivo = false;
        for (int k = 0; k < trabajos[j].numPids; ++k) {
            if (trabajos[j].pids[k] > 0) { algunoVivo = true; break; }
        }
        if (!algunoVivo) trabajos[j].estado = trabajoTerminado;
    }
}
...
static int anadirTrabajo(pid_t *pids, int num, const char *resumen) {
    ...
    for (int j = 0; j < MAX_TRABAJOS; ++j) {
        if (!trabajos[j].usado) {
            ...
            for (int k = 0; k < trabajos[j].numPids; ++k) trabajos[j].pids[k] = pids[k];
            if (resumen) {
...
static void builtinFg(int indice1) {
    ...
    for (;;) {
        ...
        for (int k = 0; k < jb->numPids; ++k) {
            if (jb->pids[k] > 0) {
                ...
                int status;
                pid_t w;
                do { w = waitpid(jb->pids[k], &status, 0); }
    ...
    jb->usado = false; jb->numPids = 0; jb->resumen[0] = '\0';
}
...
static void ejecutarLinea(tline *linea) {
    ...
    if (n > 1) {
        ...
        if (!tuberias) { perror("calloc pipes"); return; }
        for (int i = 0; i < n - 1; ++i) {
            if (pipe(tuberias[i]) < 0) {
                perror("pipe");
                for (int k = 0; k < i; ++k) { close(tuberias[k][0]); close(tuberias[k][1]); }
                free(tuberias);
                return;
            }
        }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (pid < 0) {
            perror("fork");
            if (tuberias) {
                for (int k = 0; k < n - 1; ++k) { close(tuberias[k][0]); close(tuberias[k][1]); }
                free(tuberias);
            }
            return;
        }
        ...
        if (numHijos < MAX_PROC_EN_TUBERIA) pidsHijos[numHijos++] = pid;
    }
    ...
    for (;;) {
        ...
        if (linea->ncommands == 1) {
            ...
            if (cmd0 && strcmp(cmd0, "exit") == 0) {
                int status = 0;
                if (cmd->argc >= 2) status = atoi(cmd->argv[1]);
                free(bufLinea);
                exit(status);
            }
            ...
            if (cmd0 && strcmp(cmd0, "fg") == 0) {
                ...
                if (cmd->argc < 2) {
                    ...
                    if (idx < 0) { fprintf(stderr, "fg: no hay trabajos\n"); continue; }
                } else {
                    idx = atoi(cmd->argv[1]);
                }
                builtinFg(idx);
